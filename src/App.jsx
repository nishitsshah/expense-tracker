...
const TOUR_STEPS = [
  { title:"Add an Expense", body:"Tap the + tab to log an expense. Fill in amount, narration, category and payment source.", emoji:"➕" },
  { title:"Categories", body:"Add your own categories in Settings. Drag ☰ to reorder. You can also make category optional.", emoji:"🏷️" },
  { title:"Payment Sources", body:"Add all your cards, UPI accounts and cash in Settings. Helps you track spending per account.", emoji:"💳" },
  { title:"Recurring Expenses", body:"Set up subscriptions and regular bills in Settings. Auto-added on the due date.", emoji:"🔁" },
  { title:"Summary & Insights", body:"Track spending by category, view yearly trends, and get detailed insights.", emoji:"📊" },
  { title:"Google Drive Sync", body:"Your data syncs automatically to Google Drive after every change. Works across all your devices.", emoji:"☁️" },
];
...
function SetupFlow({ googleUser, onComplete }) {
  ...
  const addCustomCat = () => {
    ...
    setSelCats(p => [...p,id]); setNewLabel(""); setNewColor("#007AFF"); setShowAddCat(false);
  };
  const addCustomPay = () => {
    if (!newLabel.trim()) return;
    const id = newLabel.toLowerCase().replace(/\s+/g,"_")+"_"+Date.now();
    setCustomPays(p => [...p,{id,label:newLabel.trim(),color:newColor}]);
    setSelPays(p => [...p,id]); setNewLabel(""); setNewColor("#007AFF"); setShowAddPay(false);
  };
  const handleComplete = () => onComplete({ categories:allCats.filter(c=>selCats.includes(c.id)), paymentSources:allPays.filter(p=>selPays.includes(p.id)), budget });
      ...
      <div style={{ flex:1, padding:"32px 24px 40px", display:"flex", flexDirection:"column" }}>
        ...
        {step===1 && (
          <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:16 }}>
                {allCats.map(c => (
                  <button key={c.id} onClick={() => toggleCat(c.id)} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 16px", borderRadius:20, border:`2px solid ${selCats.includes(c.id)?c.color:"#E5E5EA"}`, background:selCats.includes(c.id)?c.color+"22":"#fff", cursor:"pointer", fontSize:14, fontWeight:500, color:selCats.includes(c.id)?c.color:"#636366" }}>
                    <span style={{ width:8, height:8, borderRadius:"50%", background:c.color, flexShrink:0 }}/>{c.label}{selCats.includes(c.id)&&<span style={{ fontSize:12 }}>✓</span>}
                  </button>
                ))}
              ...
              {showAddCat && (
                <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:16 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>setShowAddCat(false)} style={{ flex:1, padding:"10px", borderRadius:10, border:"none", cursor:"pointer", background:"#F2F2F7", fontSize:14 }}>Cancel</button>
            ...
            <div style={{ display:"flex", gap:10, marginTop:16 }}>
              <button onClick={()=>setStep(0)} style={{ padding:"16px 24px", borderRadius:14, border:"none", cursor:"pointer", background:"#F2F2F7", fontSize:16, color:"#636366" }}>←</button>
        ...
        {step===2 && (
          <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
            ...
            <div style={{ flex:1, overflowY:"auto" }}>
              <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:16 }}>
                {allPays.map(p => (
                  <button key={p.id} onClick={()=>togglePay(p.id)} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 16px", borderRadius:20, border:`2px solid ${selPays.includes(p.id)?p.color:"#E5E5EA"}`, background:selPays.includes(p.id)?p.color+"22":"#fff", cursor:"pointer", fontSize:14, fontWeight:500, color:selPays.includes(p.id)?p.color:"#636366" }}>
                    <span style={{ width:8, height:8, borderRadius:"50%", background:p.color, flexShrink:0 }}/>{p.label}{selPays.includes(p.id)&&<span style={{ fontSize:12 }}>✓</span>}
                  </button>
                ))}
              ...
              {showAddPay && (
                <div style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:16 }}>
                  ...
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:12 }}>{COLOR_OPTIONS.map(col=><button key={col} onClick={()=>setNewColor(col)} style={{ width:28, height:28, borderRadius:"50%", background:col, border:"none", cursor:"pointer", outline:newColor===col?`3px solid ${col}`:"none", outlineOffset:2 }}/>)}</div>
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>setShowAddPay(false)} style={{ flex:1, padding:"10px", borderRadius:10, border:"none", cursor:"pointer", background:"#F2F2F7", fontSize:14 }}>Cancel</button>
            ...
            <div style={{ display:"flex", gap:10, marginTop:16 }}>
              <button onClick={()=>setStep(1)} style={{ padding:"16px 24px", borderRadius:14, border:"none", cursor:"pointer", background:"#F2F2F7", fontSize:16, color:"#636366" }}>←</button>
        ...
        {step===3 && (
          <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center" }}>
            ...
            <div style={{ display:"flex", gap:10, marginTop:8 }}>
              <button onClick={()=>setStep(2)} style={{ padding:"16px 24px", borderRadius:14, border:"none", cursor:"pointer", background:"#F2F2F7", fontSize:16, color:"#636366" }}>←</button>
              <button onClick={handleComplete} style={{ flex:1, padding:"16px", borderRadius:14, border:"none", cursor:"pointer", background:"#007AFF", color:"#fff", fontSize:16, fontWeight:600 }}>{budget?"Save & Continue →":"Skip for now →"}</button>
            </div>
          </div>
        )}
      </div>
...
export default function App() {
  ...
  const handleGoogleLogin = () => {
    const params = new URLSearchParams({ client_id:GOOGLE_CLIENT_ID, redirect_uri:REDIRECT_URI, response_type:"code", scope:GOOGLE_SCOPE, access_type:"offline", prompt:"consent" });
  ...
  const handleGoogleLogout = () => {
    setGoogleToken(null); setGoogleUser(null); setDriveFileId(null); setRefreshToken(null); setSyncStatus("idle");
  ...
  const triggerDriveSync = useCallback((overrides={}) => {
    ...
    const exp=overrides.expenses??expenses, cats=overrides.categories??categories, pays=overrides.paymentSources??paymentSources;
    const bgt=overrides.totalBudget??totalBudget, catM=overrides.categoryMandatory??categoryMandatory;
    const recur=overrides.recurringExpenses??recurringExpenses, sd=overrides.setupDone??setupDone;
  ...
  const processRecurringExpenses = () => {
    ...
    const todayStr=today(); let newExpenses=[...expenses]; let updated=false; let reminderToShow=null;
    recurringExpenses.forEach(r => {
      ...
      if (nextDate&&nextDate<=todayStr) {
        ...
        if (!alreadyAdded) { newExpenses=[{id:Date.now()+Math.random(),amount:parseFloat(r.amount),narration:r.name,category:r.category,paymentSource:r.paymentSource,date:nextDate,recurringId:r.id},...newExpenses]; updated=true; }
      }
      if (r.endDate) {
  ...
  const getNextRecurringDate = (r) => {
    const start=new Date(r.startDate+"T00:00:00"), todayD=new Date(today()+"T00:00:00");
    ...
    if (r.frequency==="Weekly") { let d=new Date(start); while(d<=todayD){const n=new Date(d);n.setDate(n.getDate()+7);if(n>todayD)break;d=n;} return f2(d); }
    if (r.frequency==="Monthly") { let d=new Date(start); while(true){const n=new Date(d);n.setMonth(n.getMonth()+1);if(n>todayD)break;d=n;} return f2(d); }
    if (r.frequency==="Quarterly") { let d=new Date(start); while(true){const n=new Date(d);n.setMonth(n.getMonth()+3);if(n>todayD)break;d=n;} return f2(d); }
    if (r.frequency==="Yearly") { let d=new Date(start); while(true){const n=new Date(d);n.setFullYear(n.getFullYear()+1);if(n>todayD)break;d=n;} return f2(d); }
    if (r.frequency==="Custom"&&r.customDays) { const days=parseInt(r.customDays); let d=new Date(start); while(true){const n=new Date(d);n.setDate(n.getDate()+days);if(n>todayD)break;d=n;} return f2(d); }
    return null;
  };
  ...
  const handleRenewal = (action) => {
    ...
    if (action==="renew") {
      const r=renewalPrompt; const oldEnd=new Date(r.endDate+"T00:00:00"); let newEnd=new Date(oldEnd);
  ...
  const handleAdd = () => {
    ...
    setExpenses(newExp); setAmount(""); setNarration(""); showToast("Expense added!"); triggerDriveSync({expenses:newExp});
  };
  ...
  const handleSaveEdit = () => {
    ...
    setExpenses(newExp); setEditExpense(null); showToast("Updated!"); triggerDriveSync({expenses:newExp});
  };
  ...
  const handleExportCSV = () => {
    ...
    else if(exportType==="fy"){const fyStart=m>=4?`${y}-04-01`:`${y-1}-04-01`;const fyEnd=m>=4?`${y+1}-03-31`:`${y}-03-31`;filtered=expenses.filter(e=>e.date>=fyStart&&e.date<=fyEnd);}
    else if(exportType==="custom"){if(!exportStart||!exportEnd)return showToast("Select date range",false);filtered=expenses.filter(e=>e.date>=exportStart&&e.date<=exportEnd);}
  ...
  const handleTouchStart=(e,i,type)=>{const timer=setTimeout(()=>{touchDragIdx.current=i;setDragIdx(i);setDragType(type);},300);e.currentTarget._dragTimer=timer;};
  ...
  const buildEmailReport=(month)=>{
    const[y,m]=month.split("-").map(Number);const monthName=`${MONTHS[m-1]} ${y}`;
    const mExp=expenses.filter(e=>monthKey(e.date)===month);const spent=mExp.reduce((s,e)=>s+e.amount,0);
  ...
  const handleEmailReport=(month)=>{
    ...
    window.location.href=`mailto:${googleUser?.email||""}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };
  ...
  // FEATURE 7: payment mode totals
  const payTotals=useMemo(()=>{const t={};monthExpenses.forEach(e=>{if(e.paymentSource)t[e.paymentSource]=(t[e.paymentSource]||0)+e.amount;});return t;},[monthExpenses]);
  ...
  // FEATURE 6: drill-down expenses for selected category
  const drillExpenses=useMemo(()=>{
    if(!drillCatId)return[];
    return monthExpenses.filter(e=>e.category===drillCatId).sort((a,b)=>b.id-a.id);
  },[drillCatId,monthExpenses]);
    ...
    <div style={{fontFamily:"-apple-system,'SF Pro Display','Helvetica Neue',sans-serif",background:"#F2F2F7",minHeight:"100vh",maxWidth:430,margin:"0 auto",position:"relative",zoom:fontScale}}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .drill-row:active{background:#F2F2F7}
      `}</style>
      ...
      <div style={{position:"sticky",top:0,zIndex:40,background:"rgba(242,242,247,.95)",backdropFilter:"blur(20px)",borderBottom:"1px solid #E5E5EA",padding:"12px 20px 10px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          {si&&(<div style={{display:"flex",alignItems:"center",gap:5,cursor:syncStatus==="error"?"pointer":"default"}} onClick={()=>syncStatus==="error"&&loadFromDrive(googleToken)}>
            <div style={{width:7,height:7,borderRadius:"50%",background:si.color,flexShrink:0}}/><span style={{fontSize:11,color:si.color}}>{si.text}</span>
          </div>)}
        </div>
      </div>
      ...
      <div style={{paddingBottom:80}}>
        ...
        {tab==="summary"&&(
          <div style={{padding:"20px"}}>
            ...
            <MonthNav value={selectedMonth} onChange={setSelectedMonth}/>
            <SubToggle value={summaryTab} onChange={v=>{setSummaryTab(v);setDrillCatId(null);}} options={[["monthly","Monthly"],["yearly","Yearly"],["insights","Insights"]]}/>
            ...
            {summaryTab==="monthly"&&(monthExpenses.length===0?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {/* FEATURE 6: By Category with tap-to-drill-down */}
                <div className="card" style={{padding:16}}>
                  ...
                  {Object.entries(catTotals).sort((a,b)=>b[1]-a[1]).map(([cid,spent])=>{
                    ...
                    const isActive=drillCatId===cid;
                    return(
                      <div key={cid}>
                        <div onClick={()=>setDrillCatId(isActive?null:cid)} style={{cursor:"pointer",borderRadius:10,padding:"6px 8px",margin:"0 -8px",background:isActive?c.color+"11":"transparent",transition:"background .15s",marginBottom:isActive?4:14}}>
                          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                            ...
                            <span style={{fontSize:12,color:c.color,marginLeft:4,display:"inline-block",transform:isActive?"rotate(90deg)":"rotate(0deg)",transition:"transform .2s"}}>›</span>
                          </div>
                          <div style={{height:5,background:"#F2F2F7",borderRadius:99,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:c.color,borderRadius:99}}/></div>
                        </div>
                        {isActive&&(
                          <div style={{marginBottom:14,borderRadius:12,overflow:"hidden",border:`1.5px solid ${c.color}33`}}>
                            {drillExpenses.length===0
                              ?<div style={{padding:"14px 16px",color:"#8E8E93",fontSize:14}}>No expenses</div>
                              :drillExpenses.map((e,i)=>{
                                const p=getPay(e.paymentSource);
                                return(
                                  <div key={e.id} className="drill-row" onClick={()=>setEditExpense({...e,amount:String(e.amount)})} style={{display:"flex",alignItems:"center",padding:"11px 14px",gap:10,borderBottom:i<drillExpenses.length-1?`1px solid ${c.color}22`:"none",cursor:"pointer"}}>
                                    <div style={{flex:1,minWidth:0}}>
                                      <div style={{fontWeight:500,fontSize:14,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{e.narration}</div>
                                      <div style={{fontSize:11,color:"#8E8E93",marginTop:2,display:"flex",gap:5}}>
                                        <span>{new Date(e.date+"T00:00:00").toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</span>
                                        {e.paymentSource&&<><span>·</span><span style={{color:p.color}}>{p.label}</span></>}
                                      </div>
                                    </div>
                                    <div style={{fontWeight:600,fontSize:14,color:c.color,flexShrink:0}}>{fmt(e.amount)}</div>
                                  </div>
                                );
                              })
                            }
                            <div style={{padding:"10px 14px",background:c.color+"11",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                              <span style={{fontSize:12,color:c.color,fontWeight:500}}>{drillExpenses.length} transaction{drillExpenses.length!==1?"s":""}</span>
                              <span style={{fontSize:14,fontWeight:700,color:c.color}}>{fmt(drillExpenses.reduce((s,e)=>s+e.amount,0))}</span>
                            </div>
                          </div>
                        )}
                      </div>
            ...
            {summaryTab==="insights"&&(!insightsData?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {/* FEATURE 7: Spending by payment mode */}
                {Object.keys(payTotals).length>0&&(
                  <div className="card" style={{padding:16}}>
                    <div style={{fontSize:13,color:"#8E8E93",marginBottom:12}}>SPENDING BY PAYMENT MODE</div>
                    {Object.entries(payTotals).sort((a,b)=>b[1]-a[1]).map(([pid,amt])=>{
                      const p=getPay(pid);const pct=totalSpent>0?(amt/totalSpent)*100:0;
                      const cnt=monthExpenses.filter(e=>e.paymentSource===pid).length;
                      return(
                        <div key={pid} style={{marginBottom:14}}>
                          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                            <span style={{width:10,height:10,borderRadius:"50%",background:p.color,flexShrink:0}}/>
                            <span style={{flex:1,fontSize:14,fontWeight:500}}>{p.label}</span>
                            <span style={{fontSize:12,color:"#8E8E93"}}>({cnt})</span>
                            <span style={{fontSize:12,color:"#8E8E93",marginLeft:4}}>{Math.round(pct)}%</span>
                            <span style={{fontSize:14,fontWeight:600,marginLeft:6}}>{fmt(amt)}</span>
                          </div>
                          <div style={{height:5,background:"#F2F2F7",borderRadius:99,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:p.color,borderRadius:99}}/></div>
                        </div>
                      );
                    })}
                  </div>
            ...
            {settingsTab==="preferences"&&(
              <div style={{display:"flex",flexDirection:"column",gap:20}}>
                ...
                <div>
                  ...
                  <div className="card"><div style={{display:"flex",alignItems:"center",padding:"16px",justifyContent:"space-between"}}>
            ...
            {settingsTab==="data"&&(
              <div style={{display:"flex",flexDirection:"column",gap:20}}>
                <div>
                  ...
                  <div className="card"><button onClick={()=>setShowExportModal(true)} style={{width:"100%",padding:"16px",background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                ...
                <div>
                  ...
                  <div className="card"><div style={{display:"flex"}}>
                ...
                <div style={{fontSize:12,color:"#C7C7CC",textAlign:"center",paddingTop:8}}>{expenses.length} total transactions · Data stored in your Google Drive</div>
              </div>
            )}
          </div>
        )}
      ...
      {/* MODALS */}
      {renewalPrompt&&(<div className="modal-bg"><div className="modal-sheet" style={{textAlign:"center"}}><div className="sheet-handle"/><div style={{fontSize:40,marginBottom:12}}>⏰</div><div style={{fontWeight:700,fontSize:20,marginBottom:8}}>{renewalPrompt.name}</div><div style={{color:"#8E8E93",fontSize:15,marginBottom:6}}>{fmt(renewalPrompt.amount)} · {renewalPrompt.frequency}</div><div style={{color:renewalPrompt.daysLeft===0?"#FF3B30":"#FF9500",fontSize:15,fontWeight:500,marginBottom:24}}>{renewalPrompt.daysLeft===0?"Expires today!":renewalPrompt.daysLeft===1?"Expires tomorrow":`Expires in ${renewalPrompt.daysLeft} days`}</div><div style={{display:"flex",flexDirection:"column",gap:10}}><button className="pill-btn" style={{width:"100%",background:"#007AFF",color:"#fff"}} onClick={()=>handleRenewal("renew")}>✓ Renew</button><button className="pill-btn" style={{width:"100%",background:"#FFE5E5",color:"#FF3B30"}} onClick={()=>handleRenewal("cancel")}>✕ Cancel Recurring</button><button className="pill-btn" style={{width:"100%",background:"#F2F2F7",color:"#636366"}} onClick={()=>setRenewalPrompt(null)}>Remind Me Later</button></div></div></div>)}
      {showExportModal&&(<div className="modal-bg" onClick={()=>setShowExportModal(false)}><div className="modal-sheet" onClick={e=>e.stopPropagation()}><div className="sheet-handle"/><div style={{fontWeight:700,fontSize:20,marginBottom:20}}>Export CSV</div><div style={{display:"flex",flexDirection:"column",gap:14}}><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:8}}>PERIOD</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{[["month","This Month"],["fy","Financial Year (Apr–Mar)"],["custom","Custom Date Range"],["all","All Time"]].map(([v,l])=>(<button key={v} onClick={()=>setExportType(v)} style={{padding:"13px 16px",borderRadius:12,border:`2px solid ${exportType===v?"#007AFF":"#E5E5EA"}`,background:exportType===v?"#007AFF11":"#fff",cursor:"pointer",textAlign:"left",fontSize:15,fontWeight:exportType===v?600:400,color:exportType===v?"#007AFF":"#1C1C1E"}}>{l}</button>))}</div></div>{exportType==="month"&&(<div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>SELECT MONTH</div><input className="ios-input" type="month" value={exportMonth} onChange={e=>setExportMonth(e.target.value)}/></div>)}{exportType==="custom"&&(<div style={{display:"flex",gap:10}}><div style={{flex:1}}><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>FROM</div><input className="ios-input" type="date" value={exportStart} onChange={e=>setExportStart(e.target.value)}/></div><div style={{flex:1}}><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>TO</div><input className="ios-input" type="date" value={exportEnd} onChange={e=>setExportEnd(e.target.value)}/></div></div>)}<div style={{display:"flex",gap:10,marginTop:4}}><button className="pill-btn" style={{flex:1,background:"#F2F2F7",color:"#1C1C1E"}} onClick={()=>setShowExportModal(false)}>Cancel</button><button className="pill-btn" style={{flex:1,background:"#007AFF",color:"#fff"}} onClick={handleExportCSV}>Export CSV</button></div></div></div></div>)}
      {showRecurringForm&&(<div className="modal-bg" onClick={()=>setShowRecurringForm(false)}><div className="modal-sheet" onClick={e=>e.stopPropagation()}><div className="sheet-handle"/><div style={{fontWeight:700,fontSize:20,marginBottom:20}}>{editRecurring?"Edit Recurring":"New Recurring Expense"}</div><div style={{display:"flex",flexDirection:"column",gap:14}}><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>NAME</div><input className="ios-input" placeholder="e.g. Netflix, Rent, Gym" value={recurringForm.name} onChange={e=>setRecurringForm(p=>({...p,name:e.target.value}))}/></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>AMOUNT</div><div style={{position:"relative"}}><span style={{position:"absolute",left:16,top:"50%",transform:"translateY(-50%)",color:"#8E8E93",fontSize:16}}>₹</span><input className="ios-input" style={{paddingLeft:34}} type="number" min="0" placeholder="0" value={recurringForm.amount} onChange={e=>setRecurringForm(p=>({...p,amount:e.target.value}))}/></div></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:8}}>CATEGORY</div><div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>{categories.map(c=>(<button key={c.id} className="cat-chip" style={{background:recurringForm.category===c.id?c.color:"#fff",color:recurringForm.category===c.id?"#fff":"#636366",flexShrink:0,border:"1.5px solid "+(recurringForm.category===c.id?c.color:"#E5E5EA")}} onClick={()=>setRecurringForm(p=>({...p,category:c.id}))}><span style={{width:7,height:7,borderRadius:"50%",background:recurringForm.category===c.id?"rgba(255,255,255,.6)":c.color,flexShrink:0}}/>{c.label}</button>))}</div></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:8}}>PAYMENT SOURCE <span style={{color:"#C7C7CC",fontSize:11}}>(optional)</span></div><div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}><button className="cat-chip" style={{background:!recurringForm.paymentSource?"#636366":"#fff",color:!recurringForm.paymentSource?"#fff":"#636366",flexShrink:0,border:"1.5px solid "+(!recurringForm.paymentSource?"#636366":"#E5E5EA")}} onClick={()=>setRecurringForm(p=>({...p,paymentSource:""}))}>None</button>{paymentSources.map(p=>(<button key={p.id} className="cat-chip" style={{background:recurringForm.paymentSource===p.id?p.color:"#fff",color:recurringForm.paymentSource===p.id?"#fff":"#636366",flexShrink:0,border:"1.5px solid "+(recurringForm.paymentSource===p.id?p.color:"#E5E5EA")}} onClick={()=>setRecurringForm(f=>({...f,paymentSource:p.id}))}><span style={{width:7,height:7,borderRadius:"50%",background:recurringForm.paymentSource===p.id?"rgba(255,255,255,.6)":p.color,flexShrink:0}}/>{p.label}</button>))}</div></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>FREQUENCY</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{FREQ_OPTIONS.map(f=>(<button key={f} onClick={()=>setRecurringForm(p=>({...p,frequency:f}))} style={{padding:"9px 16px",borderRadius:20,border:`2px solid ${recurringForm.frequency===f?"#007AFF":"#E5E5EA"}`,background:recurringForm.frequency===f?"#007AFF11":"#fff",cursor:"pointer",fontSize:14,fontWeight:recurringForm.frequency===f?600:400,color:recurringForm.frequency===f?"#007AFF":"#636366"}}>{f}</button>))}</div>{recurringForm.frequency==="Custom"&&<input className="ios-input" style={{marginTop:10}} type="number" min="1" placeholder="Every how many days?" value={recurringForm.customDays} onChange={e=>setRecurringForm(p=>({...p,customDays:e.target.value}))}/>}</div><div style={{display:"flex",gap:10}}><div style={{flex:1}}><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>START DATE</div><input className="ios-input" type="date" value={recurringForm.startDate} onChange={e=>setRecurringForm(p=>({...p,startDate:e.target.value}))}/></div><div style={{flex:1}}><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>END DATE <span style={{color:"#C7C7CC",fontSize:10}}>(opt)</span></div><input className="ios-input" type="date" value={recurringForm.endDate} onChange={e=>setRecurringForm(p=>({...p,endDate:e.target.value}))}/></div></div>{recurringForm.endDate&&<div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>REMIND ME (DAYS BEFORE EXPIRY)</div><input className="ios-input" type="number" min="1" max="30" placeholder="7" value={recurringForm.reminderDays} onChange={e=>setRecurringForm(p=>({...p,reminderDays:e.target.value}))}/></div>}<div style={{display:"flex",alignItems:"center",padding:"14px 16px",background:"#fff",borderRadius:12,justifyContent:"space-between"}}><div style={{fontWeight:500,fontSize:16}}>Active</div><div onClick={()=>setRecurringForm(p=>({...p,active:!p.active}))} style={{width:51,height:31,borderRadius:99,background:recurringForm.active?"#34C759":"#E5E5EA",cursor:"pointer",position:"relative",transition:"background .2s"}}><div style={{position:"absolute",top:2,left:recurringForm.active?22:2,width:27,height:27,borderRadius:"50%",background:"#fff",boxShadow:"0 2px 6px rgba(0,0,0,.15)",transition:"left .2s"}}/></div></div><div style={{display:"flex",gap:10,marginTop:4}}>{editRecurring&&<button className="pill-btn" style={{background:"#FFE5E5",color:"#FF3B30"}} onClick={()=>{const u=recurringExpenses.filter(r=>r.id!==editRecurring.id);setRecurringExpenses(u);setShowRecurringForm(false);setEditRecurring(null);showToast("Deleted",false);triggerDriveSync({recurringExpenses:u});}}>Delete</button>}<button className="pill-btn" style={{flex:1,background:"#F2F2F7",color:"#1C1C1E"}} onClick={()=>{setShowRecurringForm(false);setEditRecurring(null);}}>Cancel</button><button className="pill-btn" style={{flex:1,background:"#007AFF",color:"#fff"}} onClick={handleSaveRecurring}>Save</button></div></div></div></div>)}
      {editExpense&&(<div className="modal-bg" onClick={()=>setEditExpense(null)}><div className="modal-sheet" onClick={e=>e.stopPropagation()}><div className="sheet-handle"/><div style={{fontWeight:700,fontSize:20,marginBottom:20}}>Edit Expense</div><div style={{display:"flex",flexDirection:"column",gap:14}}><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>AMOUNT</div><div style={{position:"relative"}}><span style={{position:"absolute",left:16,top:"50%",transform:"translateY(-50%)",fontSize:18,color:"#8E8E93"}}>₹</span><input className="ios-input" style={{paddingLeft:36,fontSize:20,fontWeight:600}} type="number" min="0" value={editExpense.amount} onChange={e=>setEditExpense(p=>({...p,amount:e.target.value}))}/></div></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>NARRATION</div><input className="ios-input" value={editExpense.narration} onChange={e=>setEditExpense(p=>({...p,narration:e.target.value}))}/></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:8}}>CATEGORY</div><div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>{categories.map(c=>(<button key={c.id} className="cat-chip" style={{background:editExpense.category===c.id?c.color:"#fff",color:editExpense.category===c.id?"#fff":"#3C3C43",flexShrink:0}} onClick={()=>setEditExpense(p=>({...p,category:c.id}))}><span style={{width:8,height:8,borderRadius:"50%",background:editExpense.category===c.id?"rgba(255,255,255,0.6)":c.color,flexShrink:0}}/>{c.label}</button>))}</div></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:8}}>PAYMENT SOURCE</div><div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}><button className="cat-chip" style={{background:!editExpense.paymentSource?"#636366":"#fff",color:!editExpense.paymentSource?"#fff":"#636366",flexShrink:0}} onClick={()=>setEditExpense(p=>({...p,paymentSource:""}))}>None</button>{paymentSources.map(p=>(<button key={p.id} className="cat-chip" style={{background:editExpense.paymentSource===p.id?p.color:"#fff",color:editExpense.paymentSource===p.id?"#fff":"#3C3C43",flexShrink:0}} onClick={()=>setEditExpense(f=>({...f,paymentSource:p.id}))}><span style={{width:8,height:8,borderRadius:"50%",background:editExpense.paymentSource===p.id?"rgba(255,255,255,0.6)":p.color,flexShrink:0}}/>{p.label}</button>))}</div></div><div><div style={{fontSize:13,color:"#8E8E93",marginBottom:6}}>DATE</div><input className="ios-input" type="date" value={editExpense.date} onChange={e=>setEditExpense(p=>({...p,date:e.target.value}))}/></div><div style={{display:"flex",gap:10,marginTop:4}}><button className="pill-btn" style={{background:"#FFE5E5",color:"#FF3B30"}} onClick={()=>{setDeleteId(editExpense.id);setEditExpense(null);}}>Delete</button><button className="pill-btn" style={{flex:1,background:"#F2F2F7",color:"#1C1C1E"}} onClick={()=>setEditExpense(null)}>Cancel</button><button className="pill-btn" style={{flex:1,background:"#007AFF",color:"#fff"}} onClick={handleSaveEdit}>Save</button></div></div></div></div>)}
